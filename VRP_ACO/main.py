...
def print_to_file(alpha, beta, rho, distance, locs, test, ant_no ):
    with open(f"results/alpha{alpha} beta{beta} rho{rho} test{test}.txt", "w") as f:
        out_str = ""
...
def test(iteration, number_of_ants, init_capacity, filename):   #try rho as 0.95 or smth
    ...
    '''
    best_distance, best_locs = VRP(iteration, 0.2, 0.9, 0.8, number_of_ants, init_capacity, filename)
    print(best_locs)
    print(best_distance)
...
if __name__ == '__main__':
    test(1000, 10, 100, "./data/optimized_31_city.txt")
